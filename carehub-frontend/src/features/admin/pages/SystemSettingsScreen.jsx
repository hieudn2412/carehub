...
function SystemSettingsScreen({ mode = 'training' }) {
  ...
  const pageTitle = isCompetencyMode ? 'Cấu hình năng lực chuyên môn' : 'Cấu hình giờ đào tạo'
  const pageDescription = isCompetencyMode
    ? 'Thiết lập điểm sàn năng lực chuyên môn áp dụng chung cho toàn viện.'
    : 'Thiết lập mục tiêu giờ đào tạo liên tục áp dụng chung cho toàn viện.'

  const handleSave = async () => {
    if (isTrainingMode && (!Number.isFinite(globalTrainingHours) || globalTrainingHours < 0.5)) {
      showToast('Mục tiêu giờ đào tạo phải từ 0,5 giờ trở lên.', 'warning')
      return
    }
    if (isTrainingMode && (!Number.isInteger(trainingWindowYears) || trainingWindowYears < 1 || trainingWindowYears > 100)) {
      showToast('Chu kỳ đào tạo phải là số nguyên từ 1 đến 100 năm.', 'warning')
      return
    }
    if (isCompetencyMode && (!Number.isFinite(competencyTargetScore) || competencyTargetScore < 0 || competencyTargetScore > 10)) {
    ...
    try {
      ...
      showToast(`Đã cập nhật ${pageTitle.toLowerCase()}.`, 'success')
    } catch (error) {
  ...
  return (
    <AppShell breadcrumbs={[{ label: 'Cấu hình hệ thống' }, { label: pageTitle }]} title={pageTitle}>

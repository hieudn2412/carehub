...
- Một form chỉ có tối đa một version `DRAFT`. Version `PUBLISHED` và `RETIRED` không sửa cấu trúc, nhưng admin vẫn có thể đổi hoặc reset điểm sàn.
...
| Danh sách công thức chỉ số | GET | `/form-scoring-configurations` |
| Cập nhật tỷ trọng/điểm sàn | PATCH | `/forms/{formId}/versions/{versionId}/scoring-configuration` |
| Theo dõi job tính lại | GET | `/form-scoring-recalculation-jobs/{jobId}` |
| Chạy lại job thất bại | POST | `/form-scoring-recalculation-jobs/{jobId}/retry` |
...
interface FormVersion {
  ...
  criticalWeightPercent: number;
  normalWeightPercent: number;
  passingScoreMode: 'DEFAULT' | 'CUSTOM';
...
### 5.1 Cấu hình tính điểm theo version

- `criticalWeightPercent` là số nguyên `0–100`; nhóm thường là `100 - criticalWeightPercent`. Chỉ gửi thay đổi này cho version `DRAFT`.
- Khi publish draft có câu trọng yếu nhưng chưa cấu hình, backend chốt `60/40`. Version chỉ có câu thường nhận `100%` cho nhóm thường.
- `passingScore` trong response luôn là điểm sàn hiệu lực trên thang 10. `passingScoreOverride = null` nghĩa là đang dùng chính xác công thức cũ.
- Điểm sàn tùy chỉnh nhận tối đa một chữ số thập phân và được sửa/reset cho `DRAFT`, `PUBLISHED`, `RETIRED`.
- Với `PUBLISHED` và `RETIRED`, PATCH trả `202`; cấu hình cũ tiếp tục có hiệu lực cho tới khi job `COMPLETED`. Frontend polling job mỗi 3 giây và cho phép retry khi `FAILED`.

```json
{
  "criticalWeightPercent": 65,
  "passingScore": { "mode": "CUSTOM", "value": 7.5 },
  "lockVersion": 3
}
```

Để reset điểm sàn, gửi `"passingScore": { "mode": "DEFAULT" }`. Không gửi `criticalWeightPercent` khi sửa version đã publish hoặc retired; backend trả `409` nếu tỷ trọng thực sự thay đổi.
